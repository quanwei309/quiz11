...
class Model():
    def __init__(self, learning_rate=0.001, batch_size=16, num_steps=32, num_words=5000, dim_embedding=128, rnn_layers=3):
        ...
        self.state_tensor = np.zeros((batch_size, dim_embedding))
    ...
    def build(self, embedding_file=None):
        ...
        self.X = tf.placeholder(
            tf.int32, shape=[None, None], name='input')
        ...
        with tf.variable_scope('rnn'):
            ...
            self.init_state = cell.zero_state(tf.shape(self.X)[0], dtype=tf.float32)
            seq_output, self.outputs_state_tensor = tf.nn.dynamic_rnn(cell, data, initial_state=self.init_state)
        ...
        with tf.variable_scope('softmax'):
            ...
            W_o = tf.get_variable('W_o', [self.state_size, self.num_words],
                                  initializer=tf.random_normal_initializer(stddev=0.01))
            b_o = tf.get_variable('b_o', [self.num_words], initializer=tf.constant_initializer(0.0))
            fcl = tf.matmul(seq_output_final, W_o) + b_o
